...
::################################################################# ADMIN REOPEN
color 0e
echo Script made by JustMili
echo  - GitHub:     EmojiTvYt2
timeout 1 >nul
...
echo  - Create an Inbound rule to allow OpenSSH
echo.
...
echo.
echo Firewall: Creating "Allow OpenSSH" Inbound Rule...
netsh advfirewall firewall add rule name="Allow OpenSSH" dir=in action=allow service=sshd protocol=tcp localport=22 remoteport=any description="Allow OpenSSH Server connections" >nul
